...
import {
  ...
  Clock, Link, Upload, ChevronDown, Image, Copy,
} from 'lucide-react';
...
function Section({ title, description, icon: Icon, children }: { title: string; description?: string; icon: any; children: React.ReactNode }) {
  return (
    <div className="border-2 border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden mb-6">
...
function ImageUpload({ value, onChange, compact }: { value: string; onChange: (v: string) => void; compact?: boolean }) {
  ...
  return (
    ...
    <div className="flex items-center gap-3">
      <div className={cn('rounded-lg border-2 border-[var(--color-border)] bg-[var(--color-bg)] overflow-hidden flex items-center justify-center text-[var(--color-text-tertiary)]', compact ? 'w-12 h-12' : 'w-28 h-16')}>
        {value ? <img src={value} alt="" className="w-full h-full object-cover" /> : <Image className="w-4 h-4" />}
      </div>
      <div className="flex flex-col gap-1.5">
        <button type="button" onClick={() => inputRef.current?.click()}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border-2 border-[var(--color-border)] text-xs font-medium text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-muted)] transition-colors">
...
function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button onClick={() => onChange(!checked)}
      ...
      <div className={cn(
        'absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-[var(--color-bg)] shadow-[var(--shadow-card)] transition-transform',
...
export default function SettingsPage() {
  ...
  // Origin-based fill link: /a/ for admin-created forms, /f/ for user forms.
  const fillUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/${settings.source === 'admin' ? 'a' : 'f'}/${settings.publicId}`;
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <Section title="General" description="Basic form information" icon={Settings}>
        <Field label="Title">
          <input type="text" value={settings.title} onChange={e => update('title', e.target.value)}
            className="w-64 px-3 py-1.5 rounded-lg border-2 border-[var(--color-border)] bg-[var(--color-bg)] text-sm text-[var(--color-text)] outline-none focus:border-[var(--color-primary)]" />
        </Field>
        <Field label="Description">
          <textarea value={settings.description} onChange={e => update('description', e.target.value)}
            className="w-64 px-3 py-1.5 rounded-lg border-2 border-[var(--color-border)] bg-[var(--color-bg)] text-sm text-[var(--color-text)] outline-none focus:border-[var(--color-primary)] resize-none" rows={2} />
        </Field>
        ...
        <Field label="Fill link" description="Public URL shared with respondents">
          <div className="flex items-center gap-2">
            <input readOnly value={fillUrl} onFocus={e => e.target.select()}
              className="w-64 px-3 py-1.5 rounded-lg border-2 border-[var(--color-border)] bg-[var(--color-bg)] text-sm text-[var(--color-text)] outline-none" />
            <button onClick={() => navigator.clipboard.writeText(fillUrl)}
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[var(--color-primary)] text-[var(--color-bg)] text-xs font-medium hover:bg-[var(--color-primary-hover)] transition-colors">
      ...
      <Section title="Access" description="Control who can access your form" icon={Lock}>
        <Field label="Visibility">
          <select value={settings.visibility} onChange={e => update('visibility', e.target.value as any)}
            className="w-40 px-3 py-1.5 rounded-lg border-2 border-[var(--color-border)] bg-[var(--color-bg)] text-sm text-[var(--color-text)] outline-none focus:border-[var(--color-primary)]">
        ...
        <Field label="Rate limit" description="Max submissions per hour">
          <input type="number" value={settings.rateLimit || ''} onChange={e => update('rateLimit', e.target.value ? Number(e.target.value) : undefined)} placeholder="No limit"
            className="w-40 px-3 py-1.5 rounded-lg border-2 border-[var(--color-border)] bg-[var(--color-bg)] text-sm text-[var(--color-text)] outline-none focus:border-[var(--color-primary)] placeholder:text-[var(--color-text-tertiary)]" />
        </Field>
      ...
      <Section title="Responses" description="Response collection behaviour" icon={MessageSquare}>
        ...
        <Field label="Redirect URL" description="Redirect after submission (optional)">
          <input type="text" value={settings.redirectUrl || ''} onChange={e => update('redirectUrl', e.target.value)} placeholder="https://"
            className="w-64 px-3 py-1.5 rounded-lg border-2 border-[var(--color-border)] bg-[var(--color-bg)] text-sm text-[var(--color-text)] outline-none focus:border-[var(--color-primary)] placeholder:text-[var(--color-text-tertiary)]" />
        </Field>
      </Section>
      ...
      <Section title="Presentation" description="Customize the look and feel" icon={Palette}>
        ...
        <Field label="Header banner" description="Full-width image shown at the top of the form">
          <ImageUpload value={settings.bannerImage || ''} onChange={v => update('bannerImage', v)} />
        </Field>
        <Field label="Logo" description="Small brand mark shown beside the title">
          <ImageUpload value={settings.logoImage || ''} onChange={v => update('logoImage', v)} compact />
        </Field>
      ...
      <div className="border border-[var(--color-error)] bg-[var(--color-surface)] overflow-hidden mb-6">
        ...
        <div className="p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={archive} disabled={busy !== null}
              className="px-4 py-2 rounded-lg border border-[var(--color-error)] text-sm font-medium text-[var(--color-error)] hover:bg-[var(--color-error-surface)] transition-colors disabled:opacity-50">
          ...
          <div className="flex items-center justify-between">
            ...
            {confirmDelete ? (
              <span className="flex items-center gap-2">
                <span className="text-xs text-[var(--color-error)]">Are you sure?</span>
                <button onClick={removeForm} disabled={busy !== null}
                  className="px-4 py-2 rounded-lg bg-[var(--color-error)] text-sm font-medium text-[var(--color-bg)] hover:opacity-90 transition-colors disabled:opacity-50">
                  {busy === 'delete' ? 'Deleting...' : 'Delete'}
                </button>
                <button onClick={() => setConfirmDelete(false)} disabled={busy !== null}
                  className="px-3 py-2 rounded-lg border-2 border-[var(--color-border)] text-sm font-medium text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-muted)] transition-colors disabled:opacity-50">
                  Cancel
                </button>
              </span>
            ) : (
              <button onClick={() => setConfirmDelete(true)}
                className="px-4 py-2 rounded-lg bg-[var(--color-error)] text-sm font-medium text-[var(--color-bg)] hover:opacity-90 transition-colors">
                Delete form
              </button>
            )}
          </div>
