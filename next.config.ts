...
const nextConfig: NextConfig = {
  transpilePackages: ['@tirbeo/ui', '@tirbeo/theme', '@tirbeo/icons', '@tirbeo/charts', '@tirbeo/utils', '@tirbeo/config', '@paper-design/shaders-react'],
  async headers() {
    return [
      {
        source: '/(.*)',
        headers: [
          ...
          { key: 'Content-Security-Policy', value: `default-src 'self'; script-src 'self' 'unsafe-inline' ${isDev ? "'unsafe-eval'" : ""}; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data:; font-src 'self' https://fonts.gstatic.com; connect-src 'self' https://api.tirbeo.app; form-action 'self'; frame-ancestors 'none'; base-uri 'self'; object-src 'none';` },
        ],
      },
    ];
  },
