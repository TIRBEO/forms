...
function adToBs(date) {
  ...
  let month = 0;
  const daysInMonth = BS_MONTHS_DAYS[year] || BS_MONTHS_DAYS[2e3];
...
function bsToAd(bs) {
  ...
  const daysInMonth = BS_MONTHS_DAYS[bs.year] || BS_MONTHS_DAYS[2e3];
  for (let m = 0; m < bs.month - 1; m++) {
    totalDays += daysInMonth[m];
  }
  totalDays += bs.day - 1;
  return new Date(AD_EPOCH.getTime() + totalDays * 24 * 60 * 60 * 1e3);
}
